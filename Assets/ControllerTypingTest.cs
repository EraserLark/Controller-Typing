...
public class ControllerTypingTest : MonoBehaviour
{
    ...
    */
    ...
    private void Awake()
    {
        ...
        typeControls.Typing.Confirm.performed += ctx => Confirm();      //Confirm which letter you want
        typeControls.Typing.Backspace.performed += ctx => Backspace();  //Delete the letter you just typed
        //controls.Gameplay.Grow.performed += ctx => Grow();

        //Getting input from buttons, opening menu
        typeControls.Typing.EastButtonMenu.performed += ctx => OpenMenu(0);     //Run function that opens menu (0 for "arrayOfStringArrays" to know which letter group to bring up)
        typeControls.Typing.EastButtonMenu.canceled += ctx => CloseMenu();

        //Getting input from left stick, movement
        typeControls.Typing.Move.performed += ctx => move = ctx.ReadValue<Vector2>();
        typeControls.Typing.Move.canceled += ctx => move = Vector2.zero;
    ...
    {
        typeControls.Typing.Enable(); //Activates all of our actions in our action map
    }

    private void OnDisable()
    {
        typeControls.Typing.Disable();
    }
}
